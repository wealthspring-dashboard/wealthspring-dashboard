...
export async function fetchHoursByUser(tokens, { startDate, endDate }) {
  ...
  // Temporary diagnostic: to check whether QuickBooks Time's own billable
  // tracking (either on the jobcode itself, or per individual timesheet
  // entry) is actually being used, capture the raw shape of both rather
  // than guessing. Safe to remove once the real Billable Hours %
  // refinement ships.
  const rawJobcodesSeen = new Map();
  let sampleTimesheetKeysLogged = false;
  ...
  async function fetchChunk(chunkStart, chunkEnd) {
    ...
    while (more) {
      ...
      for (const ts of Object.values(timesheets)) {
        ...
        if (!sampleTimesheetKeysLogged) {
          console.error('Timesheet entry field sample (first entry seen):', JSON.stringify(ts));
          sampleTimesheetKeysLogged = true;
        }
      }
      ...
      for (const [id, jobcode] of Object.entries(jobcodes)) {
        ...
        if (!rawJobcodesSeen.has(id)) {
          rawJobcodesSeen.set(id, {
            name: jobcode?.name,
            type: jobcode?.type,
  ...
  console.error('Jobcode diagnostic -- all jobcodes seen, with billable/type/active fields:', JSON.stringify(Array.from(rawJobcodesSeen.values())));
