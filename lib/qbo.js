 ...
 */
/**
 * Collects every Summary-row label found anywhere in a report tree.
 * Used purely for diagnostics when a labeled lookup (like Contract Labor
 * spend) comes up empty, so the real QuickBooks account name can be seen
 * in the logs instead of guessing blindly at label variants.
 */
function collectAllSummaryLabels(node, out = []) {
  ...
  const rows = node.Rows?.Row || node.Row;
  if (Array.isArray(rows)) {
    for (const row of rows) collectAllSummaryLabels(row, out);
  }
  return out;
}
...
export async function fetchContractLaborSpend(tokens, { startDate, endDate }) {
  ...
  const value = findReportSummaryValueByLabels(report, labels);
  if (value === null) {
    // No exact match against our known label list -- log every expense
    // line label QBO actually returned so the real account name can be
    // identified and added to the list above.
    console.error(
      'Contract Labor lookup found no match. All P&L line labels seen:',
      JSON.stringify(collectAllSummaryLabels(report))
    );
  }
  return value;
}
