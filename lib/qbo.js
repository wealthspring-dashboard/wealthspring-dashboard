...
export async function fetchContractLaborSpend(tokens, { startDate, endDate }) {
  ...
  const labels = [
    ...
    'outside services',
    'subcontractors',
    // Seen in Wealthspring's own chart of accounts -- contractor pay is
    // filed under the broader "Payroll Expenses" section even though it
    // runs through bank transfer rather than QuickBooks Payroll.
    'payroll expenses',
    'contract labor expense',
    '1099 contractors',
  ];
  const value = findReportLineItemValueByLabels(report, labels);
  if (value === null) {
    // No exact match against our known label list -- log every line item
    // QBO actually returned (leaf accounts AND subtotals, with amounts) so
    // the real account name can be identified and added to the list above.
    console.error(
      'Contract Labor lookup found no match. All P&L line items seen:',
