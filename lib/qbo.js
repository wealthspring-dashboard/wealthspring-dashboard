...
export async function fetchProfitAndLossSummary(tokens, { type = 'month', year, month, quarter, fromDate, toDate } = {}) {
  ...
  // Revenue itself: the Expenses section, Cost of Goods Sold (including
  // labor), and Other Expenses. This deliberately overlaps with Contract
  // Labor Spend on Team & Operations, which sums the same labor accounts
  // on its own -- a conscious choice: Cost Insights is meant to be a
  // complete picture of everything on the P&L, and leaving labor out
  // would make it look like something was missing rather than shown
  // elsewhere. Contract Labor Spend itself is a separate function
  // (fetchContractLaborSpend) and is entirely unaffected by this --
  // both figures stay independently accurate, they're just allowed to
  // include the same dollars on purpose.
  const expenseCategories = [
    ...findSectionCategories(report, 'expenses'),
    ...findSectionCategories(report, 'cost of goods sold'),
