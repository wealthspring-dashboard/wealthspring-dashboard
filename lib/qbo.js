...
/**
 * Temporary diagnostic: dumps the full row structure of a section matched
 * by EXACT header name (not the loose substring match that caused the
 * earlier "Payroll Expenses" false-positive) -- so the real shape of Cost
 * of Goods Sold and Other Expenses can be seen, including nesting, before
 * writing any parsing logic to include them in Cost Insights.
 */
function diagnosticDumpSection(node, exactHeaderLower, depth = 0) {
  if (!node || typeof node !== 'object') return [];
  const header = (node.Header?.ColData?.[0]?.value || '').trim().toLowerCase();
  if (header === exactHeaderLower) {
    const results = [];
    const walk = (rowNode, d) => {
      if (!rowNode) return;
      if (rowNode.ColData && !rowNode.Rows) {
        results.push({ depth: d, label: rowNode.ColData[0]?.value, value: rowNode.ColData[1]?.value, type: 'leaf' });
      }
      if (rowNode.Header?.ColData) {
        results.push({ depth: d, label: rowNode.Header.ColData[0]?.value, type: 'group-header' });
      }
      const subRows = rowNode.Rows?.Row || rowNode.Row;
      if (Array.isArray(subRows)) subRows.forEach((r) => walk(r, d + 1));
      if (rowNode.Summary?.ColData) {
        results.push({ depth: d, label: rowNode.Summary.ColData[0]?.value, value: rowNode.Summary.ColData[1]?.value, type: 'group-summary' });
      }
    };
    const rows = node.Rows?.Row || node.Row || [];
    if (Array.isArray(rows)) rows.forEach((r) => walk(r, depth + 1));
    return results;
  }
  const rows = node.Rows?.Row || node.Row;
  if (Array.isArray(rows)) {
    for (const row of rows) {
      const found = diagnosticDumpSection(row, exactHeaderLower, depth);
...
/**
 * Walks a P&L report looking for the top-level "Expenses" section
 * (exact header match, not a substring match -- a loose /expenses/i match
 * was confirmed via diagnostic to also catch "Payroll Expenses", a
 * subgroup nested inside Cost of Goods Sold for this company, which would
 * double-count labor costs already tracked separately as Contract Labor
 * Spend). Only real leaf line items are summed; any nested subgroup's own
 * summary/total row is skipped so a subtotal is never added on top of the
 * items it already totals.
 */
function findExpenseCategories(node) {
  if (!node || typeof node !== 'object') return [];
  ...
  if (header === 'expenses') {
    ...
    const walk = (rowNode) => {
      if (!rowNode) return;
      if (rowNode.ColData && !rowNode.Rows) {
        const name = rowNode.ColData[0]?.value;
        const raw = rowNode.ColData[1]?.value;
        const amount = parseFloat(raw);
        if (name && !isNaN(amount)) categories.push({ name, amount });
      }
      const subRows = rowNode.Rows?.Row || rowNode.Row;
      if (Array.isArray(subRows)) subRows.forEach(walk);
    };
    const rows = node.Rows?.Row || node.Row || [];
  ...
  if (Array.isArray(rows)) {
    for (const row of rows) {
      const found = findExpenseCategories(row);
      if (found.length > 0) return found;
    }
  }
  return [];
...
/**
 * Computes the immediately preceding period of the same type and length --
 * last month if this is a month, last quarter if this is a quarter, last
 * year if this is a year, or an equal-length window ending the day before
 * a custom range starts. Distinct from the "same period last year" YoY
 * comparisons used elsewhere in this file (clients.js, pipeline.js) --
 * this is for period-over-period ("what changed since last period"), not
 * year-over-year.
 */
export function getPreviousPeriodParams({ type, year, month, quarter, fromDate, toDate }) {
  if (type === 'month') {
    return month === 1
      ? { type: 'month', year: year - 1, month: 12 }
      : { type: 'month', year, month: month - 1 };
  }
  if (type === 'quarter') {
    return quarter === 1
      ? { type: 'quarter', year: year - 1, quarter: 4 }
      : { type: 'quarter', year, quarter: quarter - 1 };
  }
  if (type === 'year') {
    return { type: 'year', year: year - 1 };
  }
  // custom -- an equal-length window immediately preceding fromDate, not
  // overlapping with the current range.
...
export async function fetchProfitAndLossSummary(tokens, { type = 'month', year, month, quarter, fromDate, toDate } = {}) {
  ...
  const netProfitMargin =
    totalRevenue && netIncome !== null && netIncome !== undefined && totalRevenue !== 0
      ? Math.round((netIncome / totalRevenue) * 1000) / 10
      : null;

  // Operating expenses -- the P&L's "Expenses" section specifically, not
  // Cost of Goods Sold (labor costs live there and are already tracked
  // separately as Contract Labor Spend; including them here would
  // double-count the same dollars under two different dashboard metrics).
  // Temporary diagnostic: see the real Cost of Goods Sold and Other
  // Expenses structure before deciding what to include/exclude. Safe to
  // remove once the real "all costs" Cost Insights update ships.
  try {
    console.error('COGS section diagnostic (exact match):', JSON.stringify(diagnosticDumpSection(report, 'cost of goods sold')));
    console.error('Other Expenses section diagnostic (exact match):', JSON.stringify(diagnosticDumpSection(report, 'other expenses')));
  } catch (e) {
    console.error('Cost section diagnostic failed:', e.message);
  }

  const expenseCategories = findExpenseCategories(report)
    .map((c) => ({ name: c.name, amount: Math.round(c.amount * 100) / 100 }))
    .sort((a, b) => b.amount - a.amount);
  const totalOperatingExpenses = Math.round(
    expenseCategories.reduce((sum, c) => sum + c.amount, 0) * 100
  ) / 100;
  const operatingExpenseRatio =
    totalRevenue && totalRevenue !== 0
      ? Math.round((totalOperatingExpenses / totalRevenue) * 1000) / 10
      : null;
...
/**
 * Splits revenue by service line (Tax / Bookkeeping / Other) using each
 * invoice line item's actual Product/Service name -- confirmed against
 * Wealthspring's real item list: anything with "bookkeeping" in the name
 * (e.g. "Bookkeeping", "Bookkeeping Maintenance") is Bookkeeping, anything
 * with "tax" (e.g. "Annual Tax Preparation", "Extension of Taxes") is
 * Tax. Everything else -- Advisory, Accounting/Consultation, Audit, 1099
 * Filings, entity/compliance filings, admin fees, discounts, etc. -- is a
 * real, distinct "Other" bucket rather than being forced into Tax or
 * Bookkeeping. Works on historical invoices too, since it reads what was
 * actually billed rather than depending on a classification set up going
 * forward.
 */
function categorizeServiceItem(itemName) {
  const lower = (itemName || '').toLowerCase();
  if (lower.includes('bookkeeping')) return 'bookkeeping';
  if (lower.includes('tax')) return 'tax';
  return 'other';
}

export async function fetchRevenueByServiceCategory(tokens, { startDate, endDate }) {
  const query = encodeURIComponent(
    `SELECT Id, CustomerRef, Line FROM Invoice WHERE TxnDate >= '${startDate}' AND TxnDate <= '${endDate}' MAXRESULTS 1000`
  );
  const result = await qboApiGet(
    `/v3/company/${tokens.realm_id}/query?query=${query}&minorversion=65`,
    tokens
  ...
  const totals = { tax: 0, bookkeeping: 0, other: 0 };
  const byClient = new Map(); // name -> { tax, bookkeeping, other }
  ...
  for (const inv of invoices) {
    ...
    for (const line of inv.Line || []) {
      if (!line.SalesItemLineDetail) continue; // skip subtotal/discount/description-only lines
      const itemName = line.SalesItemLineDetail.ItemRef?.name;
      const amount = line.Amount || 0;
      const category = categorizeServiceItem(itemName);

      totals[category] += amount;

      if (!byClient.has(customerName)) {
        byClient.set(customerName, { tax: 0, bookkeeping: 0, other: 0 });
      }
      byClient.get(customerName)[category] += amount;
    }
  }
  ...
  const clientBreakdown = Array.from(byClient.entries())
    .map(([name, cats]) => ({
      name,
      tax: round(cats.tax),
      bookkeeping: round(cats.bookkeeping),
      other: round(cats.other),
    }))
    .sort((a, b) => (b.tax + b.bookkeeping + b.other) - (a.tax + a.bookkeeping + a.other));

  return {
    totals: { tax: round(totals.tax), bookkeeping: round(totals.bookkeeping), other: round(totals.other) },
    clientBreakdown,
  };
}
