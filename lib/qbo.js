...
/**
 * Temporary diagnostic: walks a report's row tree looking for a group
 * whose header label matches (e.g. "Expenses", "Cost of Goods Sold"),
 * and returns every row underneath it with its label, value, and whether
 * it's a leaf line item or a subgroup -- so real category names and the
 * report's actual nesting shape can be seen before writing any parsing
 * logic, rather than guessing at QuickBooks' structure.
 */
function diagnosticFindGroupRows(node, headerPattern, depth = 0, results = []) {
  ...
  if (header && headerPattern.test(header)) {
    ...
    const walk = (rowNode, d) => {
      ...
      if (rowNode.ColData) {
        results.push({
          depth: d,
          label: rowNode.ColData[0]?.value,
          value: rowNode.ColData[1]?.value,
          type: rowNode.type || 'leaf',
        });
      }
      if (rowNode.Header?.ColData) {
        results.push({
          depth: d,
          label: rowNode.Header.ColData[0]?.value,
          value: rowNode.Header.ColData[1]?.value,
          type: 'group-header',
        });
      }
      const subRows = rowNode.Rows?.Row || rowNode.Row;
      if (Array.isArray(subRows)) {
        subRows.forEach((r) => walk(r, d + 1));
      }
      if (rowNode.Summary?.ColData) {
        results.push({
          depth: d,
          label: rowNode.Summary.ColData[0]?.value,
          value: rowNode.Summary.ColData[1]?.value,
          type: 'group-summary',
        });
      }
    };
  ...
  if (Array.isArray(rows)) {
    for (const row of rows) {
      diagnosticFindGroupRows(row, headerPattern, depth, results);
    }
  }
  return results;
}
...
export async function fetchProfitAndLossSummary(tokens, { type = 'month', year, month, quarter, fromDate, toDate } = {}) {
  ...
  // Temporary diagnostic: log the real Expenses section structure before
  // writing any category-breakdown parsing logic. Safe to remove once the
  // real expense-breakdown feature ships.
  try {
    const expenseRows = diagnosticFindGroupRows(report, /expenses/i);
    console.error('Expense section diagnostic:', JSON.stringify(expenseRows));
    const cogsRows = diagnosticFindGroupRows(report, /cost of goods sold/i);
    console.error('COGS section diagnostic:', JSON.stringify(cogsRows));
  } catch (e) {
    console.error('Expense diagnostic failed:', e.message);
  }
