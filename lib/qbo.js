...
export async function fetchContractLaborSpend(tokens, { startDate, endDate }) {
  ...
  const contractLaborValue = findReportLineItemValueByLabels(report, contractLaborLabels);
  if (contractLaborValue !== null) {
    return { value: contractLaborValue, source: 'Contract Labor' };
  }

  // Not every service business books contractor pay as an operating
  // expense -- some file it as a direct Cost of Sales line instead (the
  // labor directly tied to delivering client work). Try that account
  // before giving up.
  const costOfLaborLabels = [
    'cost of labor',
    'cost of labor - cos',
    'labor costs',
    'direct labor',
  ...
  const costOfLaborValue = findReportLineItemValueByLabels(report, costOfLaborLabels);
  if (costOfLaborValue !== null) {
    return { value: costOfLaborValue, source: 'Cost of Labor' };
  }

  // Neither account matched -- log every line item QBO actually returned
  // (leaf accounts AND subtotals, with amounts) so the real account name
  // can be identified and added to the lists above.
  console.error(
    'Contract Labor / Cost of Labor lookup found no match. All P&L line items seen:',
    JSON.stringify(collectAllLineItems(report))
  );
