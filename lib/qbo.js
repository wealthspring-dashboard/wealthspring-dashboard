...
/**
 * Diagnostic: pulls the real, active Products/Services list, plus a
 * sample of recent invoices showing which item each line item actually
 * references. Used to design real Tax-vs-Bookkeeping-vs-Advisory
 * categorization from real item names and real usage patterns, rather
 * than guessing at what the services list looks like.
 */
export async function fetchServiceItemsDiagnostic(tokens) {
  const itemsQuery = encodeURIComponent(
    `SELECT Id, Name, Type, Description FROM Item WHERE Active = true MAXRESULTS 1000`
  );
  const itemsResult = await qboApiGet(
    `/v3/company/${tokens.realm_id}/query?query=${itemsQuery}&minorversion=65`,
    tokens
  );
  const items = (itemsResult?.QueryResponse?.Item || []).map((i) => ({
    name: i.Name,
    type: i.Type,
  ...
  const invoicesQuery = encodeURIComponent(
    `SELECT Id, TxnDate, CustomerRef, Line FROM Invoice ORDER BY TxnDate DESC MAXRESULTS 20`
  );
  const invoicesResult = await qboApiGet(
    `/v3/company/${tokens.realm_id}/query?query=${invoicesQuery}&minorversion=65`,
    tokens
  );
  const invoices = (invoicesResult?.QueryResponse?.Invoice || []).map((inv) => ({
    date: inv.TxnDate,
