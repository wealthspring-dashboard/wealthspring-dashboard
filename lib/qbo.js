...
/**
 * Walks a P&L report looking for a section matched by EXACT header name
 * (learned from an earlier bug: a loose substring match on "expenses"
 * also caught "Payroll Expenses", a subgroup nested inside Cost of Goods
 * Sold). Sums only real leaf line items -- any nested subgroup's own
 * summary/total row is skipped so a subtotal is never added on top of the
 * items it already totals. excludeNames (lowercase, trimmed) lets a
 * caller skip specific accounts, used so Cost of Goods Sold can be parsed
 * without double-counting labor costs already tracked as Contract Labor
 * Spend.
 */
function findSectionCategories(node, exactHeaderLower, excludeNames = new Set()) {
  ...
  if (header === exactHeaderLower) {
    ...
    const walk = (rowNode) => {
      ...
      if (rowNode.ColData && !rowNode.Rows) {
        ...
        if (name && !isNaN(amount) && !excludeNames.has(name.trim().toLowerCase())) {
  ...
  if (Array.isArray(rows)) {
    for (const row of rows) {
      const found = findSectionCategories(row, exactHeaderLower, excludeNames);
...
export async function fetchProfitAndLossSummary(tokens, { type = 'month', year, month, quarter, fromDate, toDate, laborAccounts } = {}) {
  const resolvedYear = year || new Date().getFullYear();
  const { startDate, endDate, isCurrentPeriod } = getDateRangeFor({ type, year: resolvedYear, month, quarter, fromDate, toDate });

  // laborAccounts can be pre-fetched and passed in by a caller that calls
  // this function repeatedly (ratios.js, once per month for a trend chart)
  // -- the account list doesn't change between those calls, so fetching it
  // fresh each time would mean up to 12 redundant Chart-of-Accounts
  // queries for the same unchanging answer. Callers that only need this
  // once (summary.js, team.js) can simply omit it and it's fetched here.
  const [report, resolvedLaborAccounts] = await Promise.all([
    qboApiGet(
      `/v3/company/${tokens.realm_id}/reports/ProfitAndLoss?start_date=${startDate}&end_date=${endDate}&minorversion=65`,
      tokens
    ),
    laborAccounts ? Promise.resolve(laborAccounts) : fetchLaborRelatedAccounts(tokens).catch(() => []),
  ]);
  ...
  // "Operating Expenses" covers every real cost in the P&L outside of
  // Revenue itself: the Expenses section, Cost of Goods Sold, and Other
  // Expenses (confirmed via diagnostic to be a real, if usually empty,
  // section for this company). The one deliberate exclusion: labor costs
  // already tracked separately as Contract Labor Spend on Team &
  // Operations -- same account list, same filter Contract Labor Spend
  // itself uses (excluding anything with "tax" in the name). That filter
  // being shared is what confirmed payroll-tax accounts were not actually
  // being counted anywhere before this change -- they're now correctly
  // included here instead of continuing to fall through the cracks.
  const excludedLaborNames = new Set(
    resolvedLaborAccounts
  ...
  const expenseCategories = [
    ...findSectionCategories(report, 'expenses'),
    ...findSectionCategories(report, 'cost of goods sold', excludedLaborNames),
    ...findSectionCategories(report, 'other expenses'),
  ]
