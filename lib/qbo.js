...
export async function fetchCashBalance(tokens) {
  ...
  // Temporary diagnostic: Cash Balance has been showing blank or a
  // near-zero/negative figure. Rather than guess why, log exactly which
  // accounts QuickBooks classifies as type "Bank" and what each one's
  // real balance is -- the same approach that found the real Contract
  // Labor issue instead of a second wrong guess. Safe to remove once the
  // real fix ships.
  console.error(
    'Cash Balance diagnostic -- accounts and balances:',
