...
// 1 hour -- this is now a sliding window, not a flat "log in once, stay in
// for X hours" cap. api/session-refresh.js reissues a fresh token (and
// this same expiry) every ~15 minutes while the dashboard tab is open and
// visible, so active use is never interrupted. Close the browser (or just
// stop using it) and nothing renews it anymore -- the session dies within
// an hour regardless of whatever the browser's cookie/session-restore
// behavior happens to do, which isn't something a website can fully
// control (some browsers keep "session" cookies alive across a restore).
